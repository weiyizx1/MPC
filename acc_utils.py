...
def CarModel(t, x, ACC_Controller, param):
    ...
    A, b, P, q = ACC_Controller(t, x, param)
    ## student need to complete this function ##

    var = cp.Variable(2)
    prob = cp.Problem(cp.Minimize(0.5*cp.quad_form(var, P)+ q.T @ var),
                     [A @ var <= b])
    prob.solve()
...
def sim_vehicle(ACC_Controller, param, y0):
    ...
    y[0, :] = y0
    r = integrate.ode( lambda t, x:CarModel(t, x, ACC_Controller, param) ).set_integrator("dopri5")  # choice of method
